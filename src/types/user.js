...
const { disambiguation, escapeMarkdown } = require('../util');
...
class UserArgumentType extends ArgumentType {
	...
	async validate(val, msg, arg) {
		...
		if(matches) {
			try {
				const user = await msg.client.getRESTUser(matches[1]);
	...
	parse(val, msg) {
		const matches = val.match(/^(?:<@!?)?([0-9]+)>?$/);
		if(matches) return msg.client.getRESTUser(matches[1]) || null;
