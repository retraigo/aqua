...
class AquaDispatcher {
	/**
	 * @param {AquaClient} client - Client the dispatcher is for
	 * @param {AquaRegistry} registry - Registry the dispatcher will use
	 */
	constructor(client, registry) {
		/**
		 * Client this dispatcher handles messages for
		 * @name AquaDispatcher#client
		 * @type {AquaClient}
		 * @readonly
		 */
	 ...
	 * @callback Inhibitor
	 * @param {Message} msg - Message triggering the command
	...
	async handleMessage(message, oldMessage) {
				...
				if(cmdMsg.command) {
					if(!cmdMsg.command.isEnabledIn(message.guild)) {
						if(!cmdMsg.command.unknown) {
							 ...
							 * @event AquaClient#unknownCommand
							 * @param {Message} message - Command message that triggered the command
							 */
		...
		this.cacheMessage(message, oldMessage, cmdMsg, responses);
	 ...
	 * @param {Message} cmdMsg - Command message to inhibit
	 * @return {?Inhibition}
	 * @private
	 */
	inhibit(cmdMsg) {
	 ...
	 * @param {Message} cmdMsg - Command message to cache
	 * @param {Message|Message[]} responses - Responses to the message
	 * @private
	 */
	cacheMessage(message, oldMessage, cmdMsg, responses) {
