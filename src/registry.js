...
const { Message } = require('../extender/index')(require('eris'));
const ArgumentType = require('./types/base');

/** Handles registration and searching of commands and groups */
class AquaRegistry {
	/** @param {AquaClient} [client] - Client to use  */
	constructor(client) {
		/**
		 * The client this registry is for
		 * @name AquaRegistry#client
		 * @type {AquaClient}
		 * @readonly
		 */
	 ...
	 * @return {AquaRegistry}
	 * @see {@link AquaRegistry#registerGroups}
	 */
	registerGroup(group, name, guarded) {
		...
		if(existing) {
			 ...
			 * @event AquaClient#groupRegister
			 * @param {CommandGroup} group - Group that was registered
			 * @param {AquaRegistry} registry - Registry that the group was registered to
			 */
	 ...
	 * or arrays of {@link AquaRegistry#registerGroup} parameters
	 * @return {AquaRegistry}
	 * @example
	 ...
	 * @return {AquaRegistry}
	 * @see {@link AquaRegistry#registerCommands}
	 */
	registerCommand(command) {
		...
		if(!(command instanceof Command)) throw new Error(`Invalid command object to register: ${command.name}`);
		 ...
		 * @event AquaClient#commandRegister
		 * @param {Command} command - Command that was registered
		 * @param {AquaRegistry} registry - Registry that the command was registered to
		 */
	...
	registerCommands(commands, ignoreInvalid = false) {
		...
		for(const command of commands) {
			...
			if(ignoreInvalid && !valid) {
				...
				this.client.emit('invalidCommand', command)
				continue;
			}
	 ...
	 * @return {AquaRegistry}
	 * @see {@link AquaRegistry#registerTypes}
	 */
	registerType(type) {
		/* eslint-disable new-cap */
		 ...
		 * @event AquaClient#typeRegister
		 * @param {ArgumentType} type - Argument type that was registered
		 * @param {AquaRegistry} registry - Registry that the type was registered to
		 */
	...
	registerDefaultTypes(types = {}) {
		types = {
			...
			user: true,	command: true, group: true, ...types
		};
	...
	reregisterCommand(command, oldCommand) {
		 ...
		 * @event AquaClient#commandReregister
	...
	unregisterCommand(command) {
		 ...
		 * @event AquaClient#commandUnregister
	...
	resolveCommand(command) {
		...
		if(command instanceof Message && command.command) return command.command;
