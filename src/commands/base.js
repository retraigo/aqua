...
class Command {
	 ...
	 * @property {boolean} [masterOnly=false] - Whether or not the command is usable only by an owner
	 * @property {boolean} [staffOnly=false] - Whether or not the command is usable only by an apostle
	...
	/**
	 * @param {AquaClient} client - The client the command is for
	...
	constructor(client, info) {
		 ...
		 * @type {boolean}
		 */
		this.masterOnly = Boolean(info.masterOnly);

		/**
		 * Whether the command can only be used by staff
		 * @type {boolean}
		 */
		this.staffOnly = Boolean(info.staffOnly);
	...
	hasPermission(message, ownerOverride = true) {
		if(!this.masterOnly && !this.staffOnly && !this.userPermissions) return true;
		if(ownerOverride && this.client.isMaster(message.author)) return true;


		if(this.masterOnly && (ownerOverride || !this.client.isMaster(message.author))) {
			return `Forget that you ever saw this command.`;
		}

		if(this.staffOnly && (!this.client.isStaff(message.author) && !this.client.isMaster(message.author))) {
			console.log(`${message.author.username}#${message.author.discriminator}` + " tried to use an apostle command " + message.command.name + " " + message.command.memberName)
			return `Forget that you ever saw this command.`;
		}


		
		if(message.channel.type == 0 && this.userPermissions) {
			const available = message.channel.permissionsOf(message.author.id).json
			const missing = this.userPermissions.filter(x => !available[x])
			if(missing.length > 0) {
				if(missing.length === 1) {
	...
	onBlock(message, reason, data) {
		switch(reason) {
			case 'guildOnly':
				return message.reply(`The \`${this.name}\` command can only be used in a server channel. Am I blind because I do not see this as a server channel.`);
			case 'nsfw':
				return message.reply(`The \`${this.name}\` command is labelled NSFW and this channel is very much not an NSFW channel.`);
			case 'permission': {
			...
			case 'clientPermissions': {
				...
				return message.reply(oneLine`
					I'm missing a few permissions for the \`${this.name}\` command to work:
					${data.missing.map(perm => permissions[perm]).join(', ')}
				`);
			}
			case 'throttling': {
				return message.reply(
					`Hey hey slow down there! \`${this.name}\` command has a... uhh... ratelimit? ${data.remaining.toFixed(1)} seconds left or so I guess.`
				);
			}
			default:
				return null;
	...
	onError(err, message, args, fromPattern, result) { // eslint-disable-line no-unused-vars
		...
		return message.reply(stripIndents`
			I apparently ran into an error: \`${err.name}: ${err.message}\`
			This has been automatically reported. In case you encounter it often,
			please report this error${invite ? ` in this server: ${invite}` : '.'}
		`);
	}
	 ...
	 */
	throttle(userID) {
		if(!this.throttling || this.client.isMaster(userID) || this.client.isStaff(userID)) return null;

		let throttle = this._throttles.get(userID);
		if(!throttle) {
//			if(this.client.isPremium(userID)) {
if(false) {
				throttle = {
					start: Date.now(),
					usages: 0,
	...
	isEnabledIn(guild, bypassGroup) {
		...
		guild = this.client.guilds.get(guild.id);
	...
	unload() {
		...
		if(!require.cache[cmdPath]) throw new Error('This command is some supah important thing that you cannot get rid of.');
		delete require.cache[cmdPath];
	...
	static usage(command, prefix = null, user = null) {
		...
		return `${prefixPart || ''}${prefix && user ? '' : ''}`;
	}/*${mentionPart || ''}*/

	/**
	 * Validates the constructor parameters
	 * @param {AquaClient} client - Client to validate
